...
\rn{1} State binomial function sums to 1
\rn{2} then given the binomial expansion of $(a+b)^n=\sum_{x=o}^n \binom{n}{x} a^{n-x}b^x$
\rn{3} let $a=1-p$  and $b=p$
\rn{4} Substitute above into $(a+b)^n$
\rn{5} therefore $(1-p+p)^n=1^n=1$
...
\ra 2 Methods
\rn{1} Cumulative distribution function $F(x)$ method
...
\rn{2} Transformation method (Proposition B)

\includegraphics[scale=0.6]{./img/2fun1.jpg}

\includegraphics[scale=0.6]{./img/2fun2.jpg}
\de{Use $D$ for generating pseudorandom numbers}
...
\ssh{Functions of Jointly Distributed Random Variables}

\de{Discreet Convolution}
\includegraphics[scale=0.5]{./img/361.jpg}

\vspace{20pt}
\de{Continuous Convolution}
\includegraphics[scale=0.5]{./img/362.jpg}

\ssh{Method of cumulative distribution functions}
\includegraphics[scale=0.5]{./img/363.jpg}
